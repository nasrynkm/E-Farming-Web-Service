          ...
          if (isset($_FILES['image']) || !isset($_FILES['image'])) {
            ...
            if (move_uploaded_file($tmpName, $targetDir . $newImgName)) {
              ...
              if (in_array($imgExtension, $extensions) === true) { //CHECKING EXTENSION VALIDITY
                ...
                // IF USER IS NOT CHANGING PASSWORD 
                if (empty($oldPassword) && empty($newPassword)) {
                  // SENDING NEW DATA TO THE DATABASE TABLE
                  $selecting8 = "UPDATE users SET firstName = '$fName', lastName = '$lName', email = '$email', phone = '$phone', locationed = '$location', profilePhoto = '$newImgName' WHERE uniqueID = $userID";
                  $query8 = mysqli_query($connection, $selecting8);
                ...
                } elseif ($oldPassword == $user['passwords']) { // IF USER IS CHANGING PASSWORD 
            ...
            } elseif (!move_uploaded_file($tmpName, $targetDir . $newImgName)) {

              // IF USER IS NOT CHANGING PASSWORD 
              if (empty($oldPassword) && empty($newPassword)) {

                // SENDING NEW DATA TO THE DATABASE TABLE
                $selecting8 = "UPDATE users SET firstName = '$fName', lastName = '$lName', email = '$email', phone = '$phone', locationed = '$location' WHERE uniqueID = $userID";
                $query8 = mysqli_query($connection, $selecting8);
                ...
                // SENDING NEW DATA TO THE DATABASE TABLE WITHOUT IMAGE UPDATE 
                $selecting4 = "UPDATE users SET firstName = '$fName', lastName = '$lName', email = '$email', phone = '$phone', locationed = '$location', passwords = '$newPassword' WHERE uniqueID = $userID";
                $query4 = mysqli_query($connection, $selecting4);
